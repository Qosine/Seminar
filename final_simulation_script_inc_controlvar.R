...
true_target_params = c(-1.5, -3, 3.2, -1.0, 0.5, 1.0, .75, c(round(runif(n = 9, -2,2),1)))
true_nontarget_params = c(-2.0, 0.6, 0.4, -0.8, 0.6, 0.5, 1,c(round(runif(n = 13, -2,2),1)))
...
if(include_control_var == TRUE){
  load("./simulated_target_predictors_w_control.RData")
} else {
  load("./simulated_target_predictors.RData")
}

#simulated_target_predictors = necessary_info$target_data
simulated_targets$predictors = nthroot(add_constant(simulated_target_predictors[,-(12:13)]),3)
...
if(include_control_var == TRUE){
  load("./simulated_nontarget_predictors_w_control.RData")
} else {
  load("./simulated_nontarget_predictors.RData")
}

simulated_nontargets$predictors = nthroot(add_constant(simulated_nontarget_predictors[,-(16:17)]),3)
...
run_simulation <- function(N, Q, reps,
  ...
  beta.glm_target = data.frame(matrix(0, reps, length(true_target_params)))
  beta.glm_nontarget = data.frame(matrix(0, reps, length(true_nontarget_params)))
...
df_cols1 = c("constant","v_audiosum", "v_digitalsum", "v_programsum",
             "v_tvsum", "v_vodsum", "v_yousum","v_havechildren","v_etn_cauc","v_income030","v_income3050","v_income5075","v_educ2","v_educ3","v_married","v_single" )
df_cols2 = c("constant","v_audiosum", "v_digitalsum", "v_programsum",
            "v_tvsum", "v_vodsum", "v_yousum","v_male","v_age3544","v_age4554","v_age55plus","v_havechildren","v_etn_cauc","v_income030","v_income3050","v_income5075","v_educ2","v_educ3","v_married","v_single" )
glm_target_results_df = data.frame(matrix(0, length(df_rows), length(df_cols1)))
rownames(glm_target_results_df) = df_rows; colnames(glm_target_results_df) = df_cols1
glm_nontarget_results_df = data.frame(matrix(0, length(df_rows), length(df_cols2)))
rownames(glm_nontarget_results_df) = df_rows; colnames(glm_nontarget_results_df) = df_cols2
...
round(100*glm_target_results_df,1)[1:5,]
round(100*glm_nontarget_results_df,1)[1:5,]
