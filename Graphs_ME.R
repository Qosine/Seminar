...
x_bar = colMeans(add_constant(separate_predictors_responses(source_data)$predictors))
...
for(i in c){
  ...
  contact_betas_pop_true = true_population_params
  contact_betas_targ_true = target_params_w_Dem
  
  if (exists("P10_ratio")) {
    correct_ratio = P10_ratio
  } else if (exists("P20_ratio")) {
    correct_ratio = P20_ratio
  } else if (exists("P40_ratio")) {
    correct_ratio = P40_ratio
  }
  
  IA_total_audience = correct_IA_total_estimates(correct_ratio,
  ...
  contact_betas_pop_svyglm = colMeans(estimates$svyglm.total_audience, na.rm = TRUE)
  contact_betas_pop_IA = colMeans(IA_total_audience, na.rm = TRUE)
  contact_betas_targ_IA = colMeans(estimates$glm.interaction_target_audience, na.rm = TRUE)
  
  ME_pop_true = marginal_effects(x_bar, contact_betas_pop_true)
  ME_pop_svyglm = marginal_effects(x_bar, contact_betas_pop_svyglm)
  ME_pop_interaction = marginal_effects(x_bar, contact_betas_pop_IA)
  
  ME_targ_true = marginal_effects(x_bar, contact_betas_targ_true)
  ME_targ_interaction = marginal_effects(x_bar, contact_betas_targ_IA)
  
  svyglm_res[i,] <- abs( (ME_pop_svyglm - ME_pop_true)/ME_pop_true )
  total_w_interaction_res[i,] <- abs( (ME_pop_interaction - ME_pop_true)/ME_pop_true )
  target_w_interaction_res[i,] <- abs( (ME_targ_interaction-ME_targ_true)/ME_targ_true )
