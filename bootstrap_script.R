...
demographics_params = c(.31, .96, .71, -.55, -.66, .99, -.32, 1.44, 1.04,
...
run_bootstrap <- function(X_wo_demographics, X_w_demographics, X_digiprog_summed,
                          beta_target_wo_demographics, beta_target_w_demographics, 
                          beta_target_digiprog_summed, beta_nontarget,
  ...
  for (i in 1:n_bootstraps) {
    ...
    idx = sample(1:nrow(X_wo_demographics), sample_size, replace=T)
    
    target_wo_Dem = target_w_Dem = target_sqrt = target_sum_Digiprog = list()
    target_wo_Dem$predictors = X_wo_demographics[idx,]
    target_w_Dem$predictors = X_w_demographics[idx,]
    target_sqrt$predictors = (X_wo_demographics^(1/2))[idx,]
    target_sum_Digiprog$predictors = X_digiprog_summed[idx,]
